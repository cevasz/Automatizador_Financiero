...
## Corpus Check
- 9 files · ~139,519 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1246 nodes · 2027 edges · 128 communities (69 shown, 59 thin omitted)
...
6. `FinanceCard()` - 22 edges
7. `AgendaEntryEntity` - 20 edges
8. `IconBadge()` - 20 edges
9. `InvoiceScreen()` - 19 edges
10. `InvoiceViewModel` - 18 edges
...
- `Botón "Abonar" en Metas de ahorro (conectado, bug de SET vs incremento corregido)` --references--> `SavingsGoalDao`  [EXTRACTED]
...
- `Crear/editar presupuesto desde la UI (AddEditBudgetScreen)` --references--> `BudgetDao`  [INFERRED]
  docs/PENDIENTES.md → kivo-android/app/src/main/java/com/finanzas/automatica/data/local/dao/BudgetDao.kt
- `Reactividad Agenda/Presupuestos/Metas (Flow reactivo sobre Room)` --references--> `BudgetDao`  [EXTRACTED]
  docs/PENDIENTES.md → kivo-android/app/src/main/java/com/finanzas/automatica/data/local/dao/BudgetDao.kt
...
## Communities (128 total, 59 thin omitted)
...
Cohesion: 0.08
Nodes (18): Pantalla de gestión de categorías propias (diferido), UI para reglas de clasificación (diferido), CategoryDao, Flow, Int, List, Long, String (+10 more)

### Community 3 - "Agenda Data Access"
Cohesion: 0.05
Nodes (30): Application, Alertas y detección de patrones de gasto (§6.7), Aprendizaje comunitario opt-in de la agenda financiera (§6.2), Cifrado en reposo de la base de datos (§8.1), Confirmación ligera con gesto swipe en Movimientos (§6.10), Escaneo de comprobantes por OCR con ML Kit (§6.8), Huella financiera generativa (arte único de patrones de gasto), Instrumentar métricas de éxito del MVP (§13 SDD) (+22 more)

### Community 4 - "Invoice/Debt Data Access"
Cohesion: 0.06
Nodes (29): Bundle, Class, Context, FragmentActivity, BiometricLockGate(), BiometricLockPrompt(), showPrompt(), BiometricSettingsViewModelFactory (+21 more)
...
Cohesion: 0.17
Nodes (20): Boolean, Color, Composable, Celebración a pantalla completa al completar Meta de ahorro (peak-end rule), FinanceCard/IconBadge rediseño (esquinas, sombra, zona de toque), 5 ilustraciones de marca conectadas a composables (antes sin uso), Skill mobile-app-ui-design instalado (rediseño visual + animaciones), ImageVector (+12 more)

### Community 16 - "Budgets ViewModel"
Cohesion: 0.16
Nodes (21): Agenda: botones "Agregar contacto" y tocar contacto (ruta huérfana conectada), Movimientos: botón "Detalle" (recategorizar vía correctMovement()), Reactividad Agenda/Presupuestos/Metas (Flow reactivo sobre Room), FinanzasDatabase, Agenda, agendaEditRoute(), AppNavHost(), budgetEditRoute() (+13 more)
...
Cohesion: 0.21
Nodes (8): BudgetEntity, BudgetDetailScreen (legacy, reemplazado por AddEditBudgetScreen), Crear/editar presupuesto desde la UI (AddEditBudgetScreen), Flow, BudgetDao, Int, List, Long
...
Cohesion: 0.21
Nodes (16): DebtSummary, Total/deudas de Facturas no se actualizaban (derivedStateOf fix), Invoice, InvoiceItem, DebtSummaryCard(), InvoiceItemEditorCard(), InvoiceScreen(), InvoiceTab (+8 more)

### Community 26 - "Room Type Converters"
Cohesion: 0.26
Nodes (15): Botón "Abonar" en Metas de ahorro (conectado, bug de SET vs incremento corregido), AddEditSavingsGoalScreen(), AddProgressDialog(), GoalCelebrationOverlay(), List, Long, Modifier, NumberFormat (+7 more)
...
Cohesion: 0.21
Nodes (11): toDomain(), AgendaSource, AUTO_LEARNED, COMMUNITY_SUGGESTED, MANUAL, Budget, ClassificationRule, Failure (+3 more)
...
Cohesion: 0.25
Nodes (6): Double, Instant, Long, ParseResult, PaymentMethod, RawMovement
...
### Community 77 - "Metricas de exito del MVP"
Cohesion: 0.67
...
## Knowledge Gaps
- **162 isolated node(s):** `AGENDA`, `RULES`, `KEYWORDS`, `HISTORY`, `UNKNOWN` (+157 more)
...
- **Why does `FinanzasDatabase` connect `Agenda Data Access` to `Android/Kotlin Common Types`, `Movement Data Access (DAO)`, `In-App Notification DAO`?**
  _High betweenness centrality (0.267) - this node is a cross-community bridge._
- **Why does `docs/SDD.md` connect `Agenda Data Access` to `Bank Statement Importer`?**
  _High betweenness centrality (0.181) - this node is a cross-community bridge._
- **Why does `Escaneo de comprobantes por OCR con ML Kit (§6.8)` connect `Agenda Data Access` to `Bank Parsers & Raw Movement Model`, `Movement Processor Service`?**
